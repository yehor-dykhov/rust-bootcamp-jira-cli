...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        let mut state: DBState = self.read_db().unwrap();
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Not exist epic id."))?.stories.push(new_id);

        self.database.write_db(&state).ok();
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        state.epics.get(&epic_id).ok_or_else(|| anyhow!("Not exists epic id."))?
            .stories.iter().for_each(
            |story_id| { state.stories.remove(story_id); }
        );

        state.epics.remove(&epic_id);
        self.database.write_db(&state).map_err(|_| Error)?;
        Ok(())
    }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        if let None = state.stories.remove(&story_id) {
            return Err(anyhow!("Epic with that ID not exists"));
        }

        let epic = state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("Epic with that ID not exists"))?;
        let story_index = epic.stories.iter().position(|v| v == &story_id).unwrap();
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<(), Error> {
        let mut state: DBState = self.read_db().unwrap();

        let epic_opt = state.epics.get_mut(&epic_id);

        return if let Some(epic) = epic_opt {
            epic.status = status;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<(), Error> {
        let mut state: DBState = self.read_db().unwrap();

        let story_opt = state.stories.get_mut(&story_id);

        return if let Some(story) = story_opt {
            story.status = status;
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        let db_state = serde_json::from_reader(reader)?;
