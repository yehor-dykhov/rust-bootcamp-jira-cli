...
impl Navigator {
    ...
    pub fn get_current_page(&mut self) -> Option<&Box<dyn Page>> {
        self.pages.last()
    }
    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            Action::NavigateToEpicDetail { epic_id } => {
                self.pages.push(Box::new(EpicDetail { epic_id, db: self.db.clone() }));
            }
            Action::NavigateToStoryDetail { epic_id, story_id } => {
                self.pages.push(Box::new(StoryDetail { epic_id, story_id, db: self.db.clone() }));
            }
            ...
            Action::CreateEpic => {
                ...
                self.db.create_epic(epic).with_context(|| anyhow!("Couldn't to create an Epic"))?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                let status_opt = (*self.prompts.update_status)();
                if let Some(status) = status_opt {
                    self.db.update_epic_status(epic_id, status).with_context(|| anyhow!("Couldn't to update an Epic"))?;
                }
            }
            Action::DeleteEpic { epic_id } => {
                let deleted = (*self.prompts.delete_epic)();
                if deleted {
                    self.db.delete_epic(epic_id).with_context(|| anyhow!("Couldn't to delete an Epic"))?;
                    self.pages.pop();
                }
            }
            ...
            Action::UpdateStoryStatus { story_id } => {
                let status_opt = (*self.prompts.update_status)();
                if let Some(status) = status_opt {
                    self.db.update_story_status(story_id, status).with_context(|| anyhow!("Couldn't to update a Story"))?;
                }
            }
            Action::DeleteStory { epic_id, story_id } => {
                let deleted = (*self.prompts.delete_story)();
                if deleted {
                    self.db.delete_story(epic_id, story_id).with_context(|| anyhow!("Couldn't to delete a Story"))?;
                    self.pages.pop();
                }
            }
            Action::Exit => {
